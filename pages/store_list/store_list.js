...
Page({
		...
		wx.request({
			...
			success(res) {
				if (res.data.message == 'ok') {
					...
					wx.showToast({title: '请重试', icon: 'loading'})
				}
			},
			fail(res) {
		...
		wx.request({
			...
			success(res) {
				if (res.data.message == 'ok') {
					...
					wx.hideLoading()
					wx.switchTab({url: '/pages/recycle_list/recycle_list'})
				} else {
					wx.showToast({title: '请重试', icon: 'loading'})
				}
			},
		...
		wx.showModal({
			...
			success: function(res) {
				if (res.confirm) {
					...
					wx.reLaunch({url: '/pages/login/login?sign_out=1'})
				} else if (res.cancel) {
					console.log('用户点击取消')
				}
			}
	...
	preAddStore() {
		this.setData({add_store: true})
	},
	confirmAddStore(e) {
		var name = e.detail.value.name
		if (!name) {
			wx.showToast({title: '请输入云店铺名称', icon: 'loading'})
			return
		}
		wx.showLoading({title: '加载中'})
		var self = this

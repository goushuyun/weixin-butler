			...
			wx.request({
				...
				success(res) {
					if (res.data.message == 'ok') {
						wx.showToast({title: '密码已更改', icon: 'success'})
						self.login(formData)
						self.setData({forgetPwd: false})
					} else {
						wx.showToast({title: '密码更改失败', icon: 'loading'})
						return
					}
				}
			})
		...
		wx.request({
			...
			success(res) {
				if (res.data.message == 'ok') {
					...
					wx.hideLoading()
					wx.redirectTo({url: '/pages/store_list/store_list'})
				} else {
					wx.showToast({title: '用户名或密码错误', icon: 'loading'})
				}
			},
			fail(res) {
				wx.showToast({title: 'Error', icon: 'loading'})
			}
		})
	},
	signUp(e) {
		...
		wx.request({
			...
			success(res) {
				if (res.data.message == 'ok') {
					...
					wx.showToast({title: '用户名已存在', icon: 'loading'})
					self.setData({currentPage: 0})
				} else if (res.data.message == 'codeErr') {
					wx.showToast({title: '验证码错误', icon: 'loading'})
				}
			},
			fail(res) {
				wx.showToast({title: 'Error', icon: 'loading'})
			...
			wx.request({
				...
				success(res) {
					if (res.data.code != '00000') {
						wx.showToast({title: '获取验证码失败，请重试', icon: 'loading'})
					}
					if (res.data.message == 'exist') {
						wx.showToast({title: '该用户已被注册', icon: 'success'})
						self.setData({currentPage: 0})
					}
					if (res.data.message == 'ok') {
						wx.showToast({title: '已发送，请查收短信', icon: 'success'})
						self.setData({registe_timer_disabled: true})
						self.timer(type)
					}
				}
			})
			...
			wx.request({
				...
				success(res) {
					...
					if (res.data.message == 'needRegister') {
						wx.showToast({title: '用户名不存在，请注册', icon: 'loading'})
						self.setData({currentPage: 1})
					}
					if (res.data.message == 'ok') {
						wx.showToast({title: '已发送，请查收短信', icon: 'success'})
						self.setData({update_pwd_timer_disabled: true})
						self.timer(type)
					}
				}
			})
	...
	timer(type) {
		...
		if (type == 'register') {
			if (self.data.register_timer_second > 0) {
				...
				self.setData({register_timer_second})
				setTimeout(function() {
					self.timer(type)
				}, 1000);
			} else {
				self.setData({registe_timer_disabled: false, register_timer_second: 60})
			}
		}
		if (type == 'update_pwd') {
			if (self.data.update_pwd_timer_second > 0) {
				let update_pwd_timer_second = self.data.update_pwd_timer_second - 1
				self.setData({update_pwd_timer_second})
				setTimeout(function() {
					self.timer(type)
				}, 1000);
			} else {
				self.setData({update_pwd_timer_disabled: false, update_pwd_timer_second: 60})
			}
		}
	}
})
